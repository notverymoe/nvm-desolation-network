...
pub const CANDIDATE_AXES_SIZE: usize = 6;

pub type CandidateAxes = ArrayVec<[Vec2; CANDIDATE_AXES_SIZE]>;

macro_rules! axes {
    () => { ArrayVec::from_array_empty([Vec2::ZERO, Vec2::ZERO, Vec2::ZERO, Vec2::ZERO, Vec2::ZERO, Vec2::ZERO]) };
    ($a:expr) => { ArrayVec::from_array_len([$a, Vec2::ZERO, Vec2::ZERO, Vec2::ZERO, Vec2::ZERO, Vec2::ZERO], 1) };
    ($a:expr,$b:expr) => { ArrayVec::from_array_len([$a, $b, Vec2::ZERO, Vec2::ZERO, Vec2::ZERO, Vec2::ZERO], 2) };
    ($a:expr,$b:expr,$c:expr) => { ArrayVec::from_array_len([$a, $b, $c, Vec2::ZERO, Vec2::ZERO, Vec2::ZERO], 3) };
    ($a:expr,$b:expr,$c:expr,$d:expr) => { ArrayVec::from_array_len([$a, $b, $c, $d, Vec2::ZERO, Vec2::ZERO], 4) };
    ($a:expr,$b:expr,$c:expr,$d:expr,$e:expr) => { ArrayVec::from_array_len([$a, $b, $c, $d, $e, Vec2::ZERO], 5) };
    ($a:expr,$b:expr,$c:expr,$d:expr,$e:expr,$f:expr) => { ArrayVec::from([$a, $b, $c, $d, $e, $f]) };
}

pub fn find_static_seperating_candidates_between(a: &Shape, b: &Shape) -> CandidateAxes {    
    match (a, b) {
        (  Shape::Point(_),   Shape::Point(_)) |
        ( Shape::Circle(_),  Shape::Circle(_)) |
        (   Shape::Rect(_),    Shape::Rect(_)) |
        (Shape::Capsule(_), Shape::Capsule(_)) |
        (  Shape::Point(_),  Shape::Circle(_)) | ( Shape::Circle(_),  Shape::Point(_)) |
        (  Shape::Point(_),    Shape::Rect(_)) | (   Shape::Rect(_),  Shape::Point(_)) |
        (  Shape::Point(_), Shape::Capsule(_)) | (Shape::Capsule(_),  Shape::Point(_)) |
        ( Shape::Circle(_),    Shape::Rect(_)) | (   Shape::Rect(_), Shape::Circle(_)) |
        ( Shape::Circle(_), Shape::Capsule(_)) | (Shape::Capsule(_), Shape::Circle(_)) |
        (   Shape::Rect(_), Shape::Capsule(_)) | (Shape::Capsule(_),   Shape::Rect(_)) => axes!(),
        ...
        (Shape::Line(a),  Shape::Circle(_)) | ( Shape::Circle(_), Shape::Line(a)) => axes!(a.test_axis()),
        (Shape::Line(a),    Shape::Rect(_)) | (   Shape::Rect(_), Shape::Line(a)) => axes!(a.test_axis()),
        (Shape::Line(a), Shape::Capsule(_)) | (Shape::Capsule(_), Shape::Line(a)) => axes!(a.test_axis()),
        (Shape::Line(a),   Shape::Slope(b)) | (  Shape::Slope(b), Shape::Line(a)) => axes!(a.test_axis(), b.normal()),

        (Shape::Circle(_),  Shape::Slope(b)) | (Shape::Slope(b),  Shape::Circle(_)) => axes!(b.normal()),

        (Shape::Rect(_),    Shape::Slope(b)) | (Shape::Slope(b),    Shape::Rect(_)) => axes!(b.normal()),

        (Shape::Capsule(_), Shape::Slope(b)) | (Shape::Slope(b), Shape::Capsule(_)) => axes!(b.normal()),
    }
}

pub fn find_dynamic_seperating_candidates_between(a: &Shape, b: &Shape) -> CandidateAxes {
    match (a, b) {
        (Shape::Point(_), Shape::Point(_)) |
        ...
        (Shape::Circle(a), Shape::Capsule(b)) | (Shape::Capsule(b), Shape::Circle(a)) => axes!(axis_between(a.origin, b.start), axis_between(a.origin, b.end())),
